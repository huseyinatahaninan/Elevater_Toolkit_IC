...
class Classifier(torch.nn.Module):
    ...
    def __init__(self, config, l2_lambda):
        ...
        if not config.TRAIN.USE_CHANNEL_BN:
            self.channel_bn = torch.nn.GroupNorm(16, input_dim, affine=True)
...
def train_task(train_dataloader, test_dataloader, config, sweep_run=False):
    ...
    privacy_engine = PrivacyEngine(
        ...
        target_delta=1.0/len(train_dataloader.dataset),
        target_epsilon=config.TRAIN.TARGET_EPSILON, 
        epochs=config.TRAIN.END_EPOCH,
        max_grad_norm=config.TRAIN.MAX_GRAD_NORM,
    )
    logging.info(f"Noise multiplier: {privacy_engine.noise_multiplier}")
...
def train_one(train_loader, model, criterion, optimizer, epoch, config):
    ...
    try:
        ...
        metric_result = 0.
    eps, alpha = optimizer.privacy_engine.get_privacy_spent(optimizer.privacy_engine.target_delta)
    logging.info(f'[Epoch {epoch}] [Epsilon {eps}] Train: {metric_name} {metric_result:.3f}')
...
def full_model_finetune_dp(train_dataloader, val_dataloader, test_dataloader, args, config):

    if args.no_tuning:
        best_lr = args.lr
        best_l2_lambda = args.l2
    else:
