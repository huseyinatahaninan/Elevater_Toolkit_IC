...
from vision_benchmark.evaluation import construct_dataloader, full_model_finetune_dp
...
def add_linear_probing_args(parser):
    ...
    parser.add_argument('--max_grad_norm', help='Per sample max gradient norm', default=0.1, type=float)
...
def main():
    ...
    best_acc, model_info = full_model_finetune_dp(train_dataloader, val_dataloader, test_dataloader, args, config)
    logging.info(f'TRAINING ENDS! Best accuracy is {best_acc}')
